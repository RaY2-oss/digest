...
Запускается по cron в воскресенье сразу за последним сбором суток: одна
строка crontab, «сбор 18:00 UTC; обработчик». Отдельного часа у него нет
намеренно — сбор идёт 20-25 минут, и любое угаданное время либо режет корпус
на середине сбора, либо ждёт впустую.
...
# Пять делений с 17.08.2026 (daily_collector._JUDGE_SYSTEM). Старые строки со
# шкалой 1..3 переписаны в базе одним UPDATE по якорям: 3 -> 5, 2 -> 3, 1 -> 1.
# Держать здесь обе шкалы разом нельзя — в одной строке цифра 3 означала бы
# и «верх», и «середину», а какая именно, из строки не видно.
_SCALE_VALUE = {1: 0.0, 2: 0.25, 3: 0.5, 4: 0.75, 5: 1.0}
...
def _importance(subset: list, embeddings: np.ndarray,
    ...
    """Важность сюжетов корзины в [0, 1] — шесть факторов, без отдельного LLM.
        ...
        scale    — национальный масштаб события глазами судьи (1..5);
