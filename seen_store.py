...
Вердикты:
    ...
    prefiltered — отбросил локальный классификатор, LLM её не видела;
    pending     — решения НЕТ: провайдеры молчали или ответ не разобрался.

Окончательны первые пять. pending — это очередь, которую следующий прогон
...
Для prefiltered — тоже не сохраняем, и это НЕ экономия места. Вердикт вынесла
та самая модель, которая учится на этой таблице: запиши его как rejected с
вектором — и следующее переобучение будет подтверждать само себя, уверенно
уезжая в сторону всё более узкого отбора. Без эмбеддинга строка не может
попасть в выборку (запрос тренера требует embedding IS NOT NULL) и через
SEEN_KEEP_DAYS удаляется prune(), как прочие служебные вердикты.
"""
import sqlite3

FINAL = ("accepted", "rejected", "short", "non_event", "prefiltered")
