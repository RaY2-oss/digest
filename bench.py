...
чего выбирали. Отсюда три источника набора, а не 26 прогонов:
    ...
    2. issues   — отгруженные выпуски из output/*.docx. Файлов 30, выпусков 6:
                  ...
                  выпуску;
    3. labels   — ручная разметка владельца, bench/labels.tsv. Владелец
                  17.08.2026 решил её не делать, так что файла нет и не
                  ожидается; `bench.py labels` по-прежнему выписывает задание,
                  если решение переменится. nDCG@7 при этом НЕ подменяется
                  суррогатом: любой автоматический «эталон» здесь берётся из
                  тех же факторов, которые и оцениваются, то есть меряет сам
                  себя. Значит, все метрики ниже — косвенные: они говорят,
                  подействовала ли правка и одинаково ли, но не говорят, стал
                  ли выпуск релевантнее.
